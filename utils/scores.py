...
def cal_pesq(dirty_wavs, clean_wavs, FS=16000):
    avg_pesq_score = 0
    for i in range(len(dirty_wavs)):
        pesq_score = pesq(FS, clean_wavs[i], dirty_wavs[i], "wb")
        avg_pesq_score += pesq_score
    avg_pesq_score /= len(dirty_wavs)
    return avg_pesq_score


def cal_stoi(dirty_wavs, clean_wavs, FS=16000):
    avg_stoi_score = 0
    for i in range(len(dirty_wavs)):
        stoi_score = stoi(clean_wavs[i], dirty_wavs[i], FS, extended=False)
...
##################################################################################
# reference: https://github.com/santi-pdp/segan_pytorch/blob/master/segan/utils.py
##################################################################################
...
def llr(ref_wav, deg_wav, srate):
    ...
    for frame_count in range(num_frames):
        ...
        clean_frame = clean_frame * window
        processed_frame = processed_frame * window
        # (2) Get the autocorrection logs and LPC params used
        ...
        log_ = np.log(numerator / (denominator + 1e-7) + 1e-7)
...
def lpcoeff(speech_frame, model_order):
    ...
    for i in range(model_order):
        ...
        # print('R[i+1] = ', R[i+1])
        rcoeff[i] = (R[i + 1] - sum_term) / (E[i] + 1e-7)
        # print('len(a) = ', len(a))
